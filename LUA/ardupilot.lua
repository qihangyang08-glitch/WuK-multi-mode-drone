...
function set_motor_pwm(function_num, pwm_value)
    ...
    if SRV_Channels then
        ...
        -- 保留原有调试信息点，标准化格式
        gcs:send_text(9, string.format("LUA_DEBUG: set_motor_pwm F:%d PWM:%d", function_num, pwm_value))
        return true
    else
...
-- ==========================================================
-- 模式处理（保留原有实现，水面油门加入参数化） 
-- ==========================================================
function handle_flight_mode()
    if script_control_active then
        gcs:send_text(0, "LUA_INFO: 切换到飞行模式，归还电机控制权")
...
function handle_ground_mode()
    ...
    if throttle_active then
        local throttle_range = 2000 - GROUND_THROTTLE_THRESHOLD
        local throttle_input = throttle_pwm - GROUND_THROTTLE_THRESHOLD
        local throttle_factor = throttle_input / throttle_range
...
function handle_water_mode()
    ...
    local pitch_rad = nil
    if ahrs and ahrs.get_pitch then
        pcall(function() pitch_rad = ahrs:get_pitch() end)
    end
...
function update()
    ...
    if arming and arming.is_armed and arming:is_armed() then
        if script_control_active then
