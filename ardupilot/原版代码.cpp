...
struct HardwareConfig {
  //输入引脚
  struct {
    uint8_t modeLock = 45;      //通道10（SWA.下拨解锁）
    uint8_t modeSwitch = 2;     //通道7(SWD.上拨地面模式)
    uint8_t armControl = 3;     //通道8(SWE)
    uint8_t landingGearControl = 13; //通道9(SWF.背面为收起)（原airControl改为起落架控制）
  } inputPins;
  ...
  struct {
    struct {
      ...
      uint8_t armServos[4] = { 4, 5, 6, 7 }; //电机舵机（保留但简化用途）67对应1侧电机
...
struct StatusBar {                           //状态机
  ...
  LandingGearState landingGear = GEAR_EXTENDED; //默认起落架放下
...
void executeControlSignals() {
  ...
  if (!SystemStatus.STATE_LOCKED) {
    ...
    if (modeSignal < 1100 && SystemStatus.system != STATE_SURFACE_MODE) {
      SystemStatus.system = STATE_SURFACE_MODE;
      SystemStatus.activation = true;
    } else if (modeSignal > 1900 && SystemStatus.system != STATE_FLIGHT_MODE) {
      SystemStatus.system = STATE_FLIGHT_MODE;
      SystemStatus.activation = true;
    }
  }
...
void transitionToFlightMode() {
  ...
  if (transitionStep < totalSteps) {
    // 电机臂朝上，基础舵机转动
    int baseAngle1 = map(transitionStep, 0, totalSteps, 130, 20); //电机臂朝上
