...
// being moved
void RC_Channel_Copter::do_aux_function_change_mode(const uint8_t mode_num,
                                                    const AuxSwitchPos ch_flag)
{
    const Mode::Number mode = (Mode::Number)mode_num;
    switch(ch_flag) {
...
bool RC_Channel_Copter::do_aux_function(const AuxFuncTrigger &trigger)
{
    ...
    switch(ch_option) {
...
#if MODE_RTL_ENABLED
        case AUX_FUNC::RTL:
            do_aux_function_change_mode((uint8_t)Mode::Number::RTL, ch_flag);
            break;
#endif
        ...
        case AUX_FUNC::AUTO:
            do_aux_function_change_mode((uint8_t)Mode::Number::AUTO, ch_flag);
            break;
#endif
...
#if AUTOTUNE_ENABLED
        case AUX_FUNC::AUTOTUNE_MODE:
            do_aux_function_change_mode((uint8_t)Mode::Number::AUTOTUNE, ch_flag);
            break;
        case AUX_FUNC::AUTOTUNE_TEST_GAINS:
        ...
        case AUX_FUNC::LAND:
            do_aux_function_change_mode((uint8_t)Mode::Number::LAND, ch_flag);
            break;

        case AUX_FUNC::GUIDED:
            do_aux_function_change_mode((uint8_t)Mode::Number::GUIDED, ch_flag);
            break;

        case AUX_FUNC::LOITER:
            do_aux_function_change_mode((uint8_t)Mode::Number::LOITER, ch_flag);
            break;

        case AUX_FUNC::FOLLOW:
            do_aux_function_change_mode((uint8_t)Mode::Number::FOLLOW, ch_flag);
            break;
        ...
        case AUX_FUNC::BRAKE:
            do_aux_function_change_mode((uint8_t)Mode::Number::BRAKE, ch_flag);
            break;
#endif

#if MODE_THROW_ENABLED
        case AUX_FUNC::THROW:
            do_aux_function_change_mode((uint8_t)Mode::Number::THROW, ch_flag);
            break;
#endif
        ...
        case AUX_FUNC::SMART_RTL:
            do_aux_function_change_mode((uint8_t)Mode::Number::SMART_RTL, ch_flag);
            break;
#endif
        ...
        case AUX_FUNC::ZIGZAG:
            do_aux_function_change_mode((uint8_t)Mode::Number::ZIGZAG, ch_flag);
            break;
        ...
        case AUX_FUNC::STABILIZE:
            do_aux_function_change_mode((uint8_t)Mode::Number::STABILIZE, ch_flag);
            break;

#if MODE_POSHOLD_ENABLED
        case AUX_FUNC::POSHOLD:
            do_aux_function_change_mode((uint8_t)Mode::Number::POSHOLD, ch_flag);
            break;
#endif

        case AUX_FUNC::ALTHOLD:
            do_aux_function_change_mode((uint8_t)Mode::Number::ALT_HOLD, ch_flag);
            break;

#if MODE_ACRO_ENABLED
        case AUX_FUNC::ACRO:
            do_aux_function_change_mode((uint8_t)Mode::Number::ACRO, ch_flag);
            break;
#endif

#if MODE_FLOWHOLD_ENABLED
        case AUX_FUNC::FLOWHOLD:
            do_aux_function_change_mode((uint8_t)Mode::Number::FLOWHOLD, ch_flag);
            break;
#endif

#if MODE_CIRCLE_ENABLED
        case AUX_FUNC::CIRCLE:
            do_aux_function_change_mode((uint8_t)Mode::Number::CIRCLE, ch_flag);
            break;
#endif

#if MODE_DRIFT_ENABLED
        case AUX_FUNC::DRIFT:
            do_aux_function_change_mode((uint8_t)Mode::Number::DRIFT, ch_flag);
            break;
#endif
...
#if MODE_AUTO_ENABLED
        case AUX_FUNC::AUTO_RTL:
            do_aux_function_change_mode((uint8_t)Mode::Number::AUTO_RTL, ch_flag);
            break;
#endif

#if MODE_TURTLE_ENABLED
        case AUX_FUNC::TURTLE:
            do_aux_function_change_mode((uint8_t)Mode::Number::TURTLE, ch_flag);
            break;
#endif
