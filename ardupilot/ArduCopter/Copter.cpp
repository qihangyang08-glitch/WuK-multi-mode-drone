...
const AP_Scheduler::Task Copter::scheduler_tasks[] = {
    ...
    SCHED_TASK(uart_wuk_update,       50,    100,  7),
#if AP_FENCE_ENABLED
...
void Copter::rc_loop()
{
    ...
    // [WuK] RC9 专用模式控制，放在标准模式开关之后，避免互相覆盖
    process_wuk_switches();
}
...
{
    // [WuK-FIX-CRITICAL] 每周期强制更新电机角度，防止补偿泄漏到非MORPH模式
    update_wuk_motor_angle();
...
{
    //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "process_switch_called");
    // RC9 (Channel 8) 专用：低位 Loiter + 舵机 1100；高位启动 Morph
    RC_Channel *chan9 = rc().channel(8);
    if (chan9) {
        const uint16_t pwm9 = chan9->get_radio_in();
        static uint8_t last_state = 1; // 0:<1200, 1:中档, 2:>1900

        uint8_t state = 1;
        if (pwm9 > 1800) {
            //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "pwm9 > 1800");
            state = 2;
        } else if (pwm9 < 1200) {
            //GCS_SEND_TEXT(MAV_SEVERITY_INFO, "pwm9 < 1200");
            state = 0;
        }
    ...
    // RC7 (Channel 6) and RC8 (Channel 7) for Actuators
    static uint16_t last_rc7_val = 0;
    static uint16_t last_rc8_val = 0;
    const uint16_t deadband = 20;

    RC_Channel *chan7 = rc().channel(6);
    if (chan7) {
        uint16_t val = chan7->get_radio_in();
        if (abs(val - last_rc7_val) > deadband) {
            wuk_comms.enqueue_cmd(WuK_Comms::CmdID::ACT1, val);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "WuK RC7: %u -> ACT1", (unsigned)val);
    ...
    if (chan8) {
        uint16_t val = chan8->get_radio_in();
        if (abs(val - last_rc8_val) > deadband) {
            wuk_comms.enqueue_cmd(WuK_Comms::CmdID::ACT2, val);
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "WuK RC8: %u -> ACT2", (unsigned)val);
            last_rc8_val = val;
        }
    }
}

// [WuK-FIX-CRITICAL] 独立函数：每个周期强制重置电机角度
// 必须在高频率任务中调用（如throttle_loop 50Hz），确保非MORPH模式不残留补偿
