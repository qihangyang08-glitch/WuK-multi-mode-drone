...
struct DataPoint {
    ...
    bool controlled_descent_detected;
...
std::vector<DataPoint> parse_jsonl(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string controlled_descent = extract_json_value(events_obj, "controlled_descent");
        pt.controlled_descent_detected = (controlled_descent == "true");
...
// 过滤掉电机启动前的数据，只保留启动前一小段时间（如2秒）
void filter_pre_arm_data(std::vector<DataPoint>& data) {
    if (data.empty()) return;

    // 寻找电机启动时刻（总推力 > 1.0N）
    int start_idx = -1;
    for (size_t i = 0; i < data.size(); i++) {
        if (data[i].total_thrust > 1.0f) {
            start_idx = i;
            break;
        }
    }
    ...
    if (start_idx == -1) {
        std::cout << "⚠ 未检测到电机启动（推力始终为0），保留所有数据" << std::endl;
        return;
    }

    // 向前回溯一段时间（例如2秒）
    double start_time = data[start_idx].time_s;
    double keep_time = start_time - 2.0; // 保留启动前2秒
    
    int keep_idx = 0;
    for (int i = start_idx; i >= 0; i--) {
        if (data[i].time_s < keep_time) {
            keep_idx = i + 1;
            break;
        }
    }
    ...
    if (keep_idx > 0) {
        std::cout << "✓ 过滤预启动数据: 移除前 " << keep_idx << " 个点 (" 
                  << std::fixed << std::setprecision(2) << data[keep_idx].time_s - data[0].time_s << " 秒)" << std::endl;
        data.erase(data.begin(), data.begin() + keep_idx);
        
        // 重置时间戳，使起始时间为0
        double time_offset = data[0].time_s;
        for (auto& pt : data) {
            pt.time_s -= time_offset;
        }
    }
}
...
void write_csv(const std::string& filename, const std::vector<DataPoint>& data) {
    ...
    file << "time_s,fc_angle,morph_active,"
         ...
         << "morph_start,controlled_descent,altitude_drop\n";
    ...
    for (const auto& pt : data) {
        ...
        file << pt.total_thrust << ","
             ...
             << (pt.controlled_descent_detected ? 1 : 0) << ","
...
int main(int argc, char* argv[]) {
    ...
    float controlled_descent_time = -1.0f;
    float altitude_drop_time = -1.0f;
    for (const auto& pt : data) {
        if (morph_start_time < 0 && pt.morph_start_detected) {
            morph_start_time = pt.time_s;
        }
        if (controlled_descent_time < 0 && pt.controlled_descent_detected) {
            controlled_descent_time = pt.time_s;
        }
    ...
    if (controlled_descent_time >= 0) {
        std::cout << "✓ 受控下降 (90°): " << controlled_descent_time << " 秒" << std::endl;
    } else {
        std::cout << "⚠ 未检测到受控下降事件" << std::endl;
    }
