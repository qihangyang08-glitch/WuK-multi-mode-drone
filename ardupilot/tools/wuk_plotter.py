...
def load_data(csv_file):
    ...
    try:
        ...
        print(f"✓ Successfully loaded data: {len(df)} data points")
        return df
...
def detect_events(df):
    ...
    if len(morph_start_idx) > 0:
        ...
        print(f"✓ Detected morph start: {events['morph_start']:.2f} seconds")
    else:
        print("⚠ Morph start event not detected")

    # Controlled descent event (first time controlled_descent == 1)
    if 'controlled_descent' in df.columns:
        cd_idx = df[df['controlled_descent'] == 1].index
        if len(cd_idx) > 0:
            events['controlled_descent'] = df.loc[cd_idx[0], 'time_s']
            print(f"✓ Detected controlled descent (90deg): {events['controlled_descent']:.2f} seconds")
        else:
            print("⚠ Controlled descent event not detected")
    
    # Rapid altitude drop event
    drop_idx = df[df['altitude_drop'] == 1].index
    if len(drop_idx) > 0:
        events['altitude_drop'] = df.loc[drop_idx[0], 'time_s']
        print(f"✓ Detected altitude drop: {events['altitude_drop']:.2f} seconds")
    else:
        print("✓ No altitude drop detected (altitude control stable)")
...
def plot_data(df, events, output_file='wuk_analysis.png'):
    ...
    if events['controlled_descent']:
        ax1.axvline(x=events['controlled_descent'], color='blue', linestyle='-.', 
                   linewidth=2.5, alpha=0.7, label='Controlled Descent')
    ...
    if events['controlled_descent']:
        ax2.axvline(x=events['controlled_descent'], color='blue', linestyle='-.', 
                   linewidth=2.5, alpha=0.7)
    if events['altitude_drop']:
    ...
    if events['morph_start']:
        ax3.axvline(x=events['morph_start'], color='red', linestyle=':', 
                   linewidth=2.5, alpha=0.7)
    if events['controlled_descent']:
        ax3.axvline(x=events['controlled_descent'], color='blue', linestyle='-.', 
                   linewidth=2.5, alpha=0.7)
    if events['altitude_drop']:
    ...
    if events['morph_start']:
        ax4.axvline(x=events['morph_start'], color='red', linestyle=':', 
                   linewidth=2.5, alpha=0.7, label='Morph Start')
    if events['controlled_descent']:
        ax4.axvline(x=events['controlled_descent'], color='blue', linestyle='-.', 
                   linewidth=2.5, alpha=0.7, label='Controlled Descent')
    if events['altitude_drop']:
...
def print_summary(df, events):
    ...
    print(f"\nFC angle range: {df['fc_angle'].min():.1f}° - {df['fc_angle'].max():.1f}°")
    
    print("\nAttitude change:")
    print(f"  Roll:  [{df['roll_deg'].min():.2f}°, {df['roll_deg'].max():.2f}°]")
    print(f"  Pitch: [{df['pitch_deg'].min():.2f}°, {df['pitch_deg'].max():.2f}°]")
    print(f"  Yaw:   [{df['yaw_deg'].min():.2f}°, {df['yaw_deg'].max():.2f}°]")
    
    print("\nMotor tilt angle change (body frame):")
    for i in range(4):
        col = f'm{i+1}_tilt_body'
        print(f"  M{i+1}: [{df[col].min():.2f}°, {df[col].max():.2f}°]")
    ...
    if events['morph_start']:
        ...
        if len(pre_df) > 0 and len(post_df) > 0:
            ...
            print(f"  Avg total thrust: {pre_thrust:.2f}N → {post_thrust:.2f}N ({thrust_change:+.1f}%)")
            ...
            print(f"  Avg vertical thrust: {pre_vthrust:.2f}N → {post_vthrust:.2f}N ({vthrust_change:+.1f}%)")
            
            pre_alt = pre_df['altitude_m'].mean()
            post_alt = post_df['altitude_m'].mean()
            alt_change = post_alt - pre_alt
            print(f"  Avg altitude: {pre_alt:.2f}m → {post_alt:.2f}m ({alt_change:+.2f}m)")
    
    print("\nKey events:")
    if events['morph_start']:
        print(f"  ✓ Morph start: {events['morph_start']:.2f} seconds")
    else:
        print("  ✗ Morph start not detected")

    if events['controlled_descent']:
        print(f"  ✓ Controlled descent (90deg): {events['controlled_descent']:.2f} seconds")
    else:
        print("  ✗ Controlled descent not detected")
    
    if events['altitude_drop']:
        print(f"  ⚠ Altitude drop: {events['altitude_drop']:.2f} seconds")
    else:
        print("  ✓ No altitude drop (altitude control stable)")
...
def main():
    ...
    print("   - Blue dash-dot line: Controlled descent (90deg) time")
    print("   - Orange dashed line: Rapid altitude drop time")
    print("   - Chart 1: Motor tilt should change from 0° to 45° (MORPH) or 90° (GROUND)")
