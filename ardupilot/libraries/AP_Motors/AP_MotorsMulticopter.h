...
public:
    ...
    // WuK Morph support
    virtual void        set_morph_angle(float deg) {}
    virtual void        set_morph_comp_enabled(bool enabled) {}
    ...
    // [WuK] Public method for ground mode to output without mixing
    // Performs output steps: filter, logic, thrust_compensation, output_to_motors
    // but SKIPS output_armed_stabilizing() to preserve set_ground_thrust() values
    void output_direct_to_motors() {
        update_throttle_filter();
        thr_lin.update_lift_max_from_batt_voltage();
        output_logic();
        // [WuK] Intentionally skip output_armed_stabilizing() here
        thrust_compensation();
        output_to_motors();
