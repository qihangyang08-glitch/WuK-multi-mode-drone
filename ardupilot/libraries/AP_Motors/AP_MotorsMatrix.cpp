...
void AP_MotorsMatrix::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    // WuK Init
    _morph_angle_deg = 0.0f;
    _ground_fwd_thrust = 0.0f;
    _ground_diff_thrust = 0.0f;
}

// WuK Morph support
void AP_MotorsMatrix::set_morph_angle(float angle_deg)
{
    _morph_angle_deg = constrain_float(angle_deg, 0.0f, 90.0f);
}

void AP_MotorsMatrix::set_ground_thrust(float fwd_thrust, float diff_thrust)
{
    // [WuK-GROUND] X型布局90°构型: 所有电机水平向后推进
    // 前进控制: 四个电机同时增减推力
    // 转向控制: 左侧组(M2+M3) vs 右侧组(M1+M4)推力差
    
    // 限制输入范围
    fwd_thrust = constrain_float(fwd_thrust, 0.0f, 1.0f);
    diff_thrust = constrain_float(diff_thrust, -1.0f, 1.0f);
    
    // 计算左右侧推力
    // diff_thrust > 0: 右转(右侧推力大)
    // diff_thrust < 0: 左转(左侧推力大)
    float left_thrust = fwd_thrust - diff_thrust * 0.5f;
    float right_thrust = fwd_thrust + diff_thrust * 0.5f;
    
    // 防止推力溢出
    left_thrust = constrain_float(left_thrust, 0.0f, 1.0f);
    right_thrust = constrain_float(right_thrust, 0.0f, 1.0f);
    
    // 直接设置各电机输出 (90°构型下bypass标准mixing)
    // X型布局(quad_x_motors): M1前右, M2后左, M3前左, M4后右
    _thrust_rpyt_out[0] = right_thrust;  // M1 前右
    _thrust_rpyt_out[1] = left_thrust;   // M2 后左
    _thrust_rpyt_out[2] = left_thrust;   // M3 前左
    _thrust_rpyt_out[3] = right_thrust;  // M4 后右
    
    // 标记为地面模式(可选,用于调试)
    _throttle_in = fwd_thrust;
    _roll_in = diff_thrust;
    _pitch_in = 0.0f;
    _yaw_in = 0.0f;
}
...
void AP_MotorsMatrix::output_armed_stabilizing()
{
    ...
    // [WuK] ========== 变构补偿逻辑 START ==========
    // [WuK] 计算变构角度的三角函数值
    float cos_morph = 1.0f;
    float sin_morph = 0.0f;
    // [WuK-FIX] 严格限制补偿仅在0-45°范围内启用（排除90°GROUND模式）
    bool apply_morph = (_morph_comp_enabled && _morph_angle_deg > 0.01f && _morph_angle_deg < 45.0f);
    if (apply_morph) {
        cos_morph = cosf(radians(_morph_angle_deg));
        sin_morph = sinf(radians(_morph_angle_deg));
        
        // [WuK] 垂直推力补偿: 电机倾转后，垂直分量 = 总推力 × cos(θ)
        // 要维持垂直推力不变，需放大总推力: 总推力 /= cos(θ)
        throttle_thrust /= cos_morph;
        throttle_thrust = MIN(throttle_thrust, 1.0f);  // 防止饱和

        // 调试输出：混控补偿运行区间
        static uint32_t last_wuk_log_ms = 0;
        const uint32_t now_ms = AP_HAL::millis();
        if (now_ms - last_wuk_log_ms > 200) { // 5Hz 防刷屏
            hal.console->printf("WuK MorphComp angle=%.2f cos=%.3f thr=%.3f\n", (double)_morph_angle_deg, (double)cos_morph, (double)throttle_thrust);
            last_wuk_log_ms = now_ms;
        }
    }

    for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            // [WuK] Roll/Yaw因子旋转矩阵补偿 (推力方向校正)
            float roll_fac = _roll_factor[i];
            float yaw_fac = _yaw_factor[i];
            if (apply_morph) {
                float r = roll_fac;
                float y = yaw_fac;
                roll_fac = r * cos_morph - y * sin_morph;
                yaw_fac = y * cos_morph + r * sin_morph;
            }

            // calculate the thrust outputs for roll and pitch
            _thrust_rpyt_out[i] = roll_thrust * roll_fac + pitch_thrust * _pitch_factor[i];

            // Check the maximum yaw control that can be used on this channel
            // Exclude any lost motors if thrust boost is enabled
            if (!is_zero(yaw_fac) && (!_thrust_boost || i != _motor_lost_index)) {
                const float thrust_rp_best_throttle = throttle_thrust_best_rpy + _thrust_rpyt_out[i];
                float motor_room;
                if (is_positive(yaw_thrust * yaw_fac)) {
                    // room to upper limit
                ...
                const float motor_yaw_allowed = MAX(motor_room, 0.0)/fabsf(yaw_fac);
    ...
    for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            // WuK Morph Compensation
            float yaw_fac = _yaw_factor[i];
            if (apply_morph) {
                float r = _roll_factor[i];
                float y = yaw_fac;
                yaw_fac = y * cos_morph + r * sin_morph;
            }

            _thrust_rpyt_out[i] = _thrust_rpyt_out[i] + yaw_thrust * yaw_fac;
